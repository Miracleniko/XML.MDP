    ...
    where TEntity : Entity<TEntity>, new()
{
    ...
    private void StartTimer()
    {
        if (_Timer == null)
        {
            ...
            _Timer = new TimerEV(CheckExpire, null, period, period, "SC")
            {
                Async = true
            };
        }
    ...
    private void CheckExpire(Object state)
    {
        ...
        // 找到所有很久未访问的缓存项，10倍
        var exp = TimerEV.Now.AddSeconds(-10 * ClearPeriod);
    ...
    class CacheItem
    {
        ...
        {
            Entity = entity;
            ExpireTime = TimerEV.Now.AddSeconds(expire);
            VisitTime = ExpireTime;
        }

        /// <summary>获取已过期时间</summary>
        /// <returns></returns>
        public TimeSpan GetExpired() => TimerEV.Now - ExpireTime;
    }
    #endregion
    ...
    private void UpdateData(Object state, Object key)
    {
        ...
        // 先修改过期时间
        item.ExpireTime = TimerEV.Now.AddSeconds(Expire);
