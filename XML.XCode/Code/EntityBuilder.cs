...
public class EntityBuilder : ClassBuilder
{
    ...
    public static void FixModelFile(String xmlFile, BuilderOption option, IDictionary<String, String> atts, IList<IDataTable> tables)
    {
        ...
        // 更新xsd
        atts["xmlns"] = "https://data.xmoonlight.com/Model2022.xsd";
        atts["xs:schemaLocation"] = "https://data.xmoonlight.com https://data.xmoonlight.com/Model2022.xsd";

        // 版本和教程
        var asm = AssemblyX.Create(Assembly.GetExecutingAssembly());
        atts["Version"] = asm.FileVersion + "";
        atts["Document"] = "https://data.xmoonlight.com/xcode/model";
    ...
    protected virtual void AddNameSpace()
    {
        ...
        us.Add("XML.XCode");
        us.Add("XML.XCode.Configuration");
        us.Add("XML.XCode.DataAccessLayer");

        if (Business) us.Add("XML.XCode.Shards");
        ...
        if (Business && !Option.Pure)
        {
            ...
            us.Add("XML.Core");
            us.Add("XML.Core.Data");
            us.Add("XML.Core.Model");
            us.Add("XML.Core.Log");
            us.Add("XML.Core.Reflection");
            us.Add("XML.Core.Threading");
            us.Add("XML.Core.Web");
            us.Add("XML.XCode.Cache");
            us.Add("XML.XCode.Membership");
            us.Add("XML.XCode.Shards");
        }
    }
    ...
    protected virtual void BuildSearch()
    {
        ...
        // 字段缓存，用于前台下拉选择
        {
            ...
            foreach (var di in idx)
            {
                ...
                WriteLine("};");
                WriteLine();
                WriteLine($"/// <summary>获取{dc.DisplayName}列表，字段缓存10分钟，分组统计数据最多的前20种，用于前台下拉选择</summary>");
                WriteLine("/// <returns></returns>");
            ...
            if (count == 0)
            {
                ...
                WriteLine("//};");
                WriteLine();
                WriteLine("///// <summary>获取类别列表，字段缓存10分钟，分组统计数据最多的前20种，用于前台下拉选择</summary>");
                WriteLine("///// <returns></returns>");
