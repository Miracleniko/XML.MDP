...
/// <summary>性能追踪器，追踪数据提交到平台</summary>
...
public class StarTracer : DefaultTracer
{
    ...
    private void Init()
    {
        ...
        WriteLog("监控中心 Server={0} AppId={1} ClientId={2}", server, AppId, ClientId);
    ...
    public void AttachGlobal()
    {
        ...
        // 反射处理Cube追踪
        {
            var type = "XML.MDP.WebMiddleware.TracerMiddleware".GetTypeEx();
    ...
    #region 全局注册
    /// <summary>全局注册性能追踪器</summary>
    /// <param name="server">监控中心地址，为空时自动从本地探测</param>
    /// <returns></returns>
    public static StarTracer Register(String server = null)
    {
        ...
        if (server.IsNullOrEmpty())
        {
            ...
            if (!server.IsNullOrEmpty()) XTrace.WriteLine("探测：{0}", server);
        }
