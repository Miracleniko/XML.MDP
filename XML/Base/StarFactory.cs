...
public class StarFactory : DisposeBase
{
    ...
    #region 构造
    /// <summary>
    /// 实例化工厂，先后读取appsettings.json、本地StarAgent、star.config
    /// </summary>
    public StarFactory() => Init();

    /// <summary>实例化工厂，指定地址、应用和密钥，创建工厂</summary>
    ...
    private void Init()
    {
        XTrace.WriteLine("正在初始化……");

        Local = new LocalStarClient();

        // 从环境变量读取地址、应用Id、密钥，方便容器化部署
        ...
        try
        {
            ...
            if (!server.IsNullOrEmpty())
            {
                ...
                XTrace.WriteLine("探测：StarAgent Not Found");
        }
        catch (Exception ex)
        {
            XTrace.Log.Error("探测失败！{0}", ex.Message);
        }
        //}
        ...
        XTrace.WriteLine("分布式服务 Server={0} AppId={1} ClientId={2}", Server, AppId, ClientId);
    ...
    {
        XTrace.WriteLine("初始化监控中心，采样并定期上报应用性能埋点数据，包括Api接口、Http请求、数据库操作、Redis操作等。可用于监控系统健康状态，分析分布式系统的性能瓶颈。");
    ...
    public IConfigProvider Config
    {
        get
        {
            if (_config == null)
            {
                ...
                XTrace.WriteLine("初始化配置中心，提供集中配置管理能力，自动从配置中心加载配置数据，包括XCode数据库连接。配置中心同时支持分配应用实例的唯一WorkerId，确保Snowflake算法能够生成绝对唯一的雪花Id");
    ...
    /// <summary>设置本地配置提供者，该提供者将跟配置结合到一起，形成复合配置提供者</summary>
    ...
    public IRegistry Service
    {
        get
        {
            if (!_initService)
            {
                ...
                XTrace.WriteLine("初始化注册中心，提供服务注册与发布能力");
            }
