...
/// <summary>应用客户端。每个应用有一个客户端连接服务端</summary>
public class AppClient : ApiHttpClient, ICommandClient, IRegistry
{
    ...
    private void StartTimer()
    {
        if (_timer == null)
        {
            lock (this)
            {
                if (_timer == null)
                {
                    _timer = new TimerEV(DoPing, null, 5_000, 60_000) { Async = true };
    ...
    private async Task DoPing(Object state)
    {
        ...
        try
        {
            ...
            if (svc != null && UseWebSocket)
            {
                ...
                var token = Token;
                if (Filter is XML.Core.Http.TokenHttpFilter thf) token = thf.Token?.AccessToken;
    ...
    {
        var file = XML.Core.Setting.Current.DataPath.CombinePath("star_services.json").GetBasePath();
