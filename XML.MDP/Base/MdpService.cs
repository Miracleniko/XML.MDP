...
using static XML.Core.Remoting.ApiHttpClient;
...
/// <summary>曦月服务</summary>
public static class MdpService
{
    /// <summary>区域名集合</summary>
    public static String[] AreaNames { get; set; }

    #region 配置曦月
    /// <summary>添加曦月，放在AddControllersWithViews之后</summary>
    /// <param name="services"></param>
    /// <returns></returns>
    public static IServiceCollection AddMdp(this IServiceCollection services)
    {
        // 引入平台
        ...
        using var span = DefaultTracer.Instance?.NewSpan(nameof(AddMdp));

        XTrace.WriteLine("{0} Start 配置曦月 {0}", new String('=', 32));
        Assembly.GetExecutingAssembly().WriteVersion();

        // 修正系统名，确保可运行
        var sys = SysConfig.Current;
        if (sys.Name == "XML.MDP.Views" || sys.DisplayName == "XML.MDP.Views")
        {
            sys.Name = "XML.MDP";
            sys.DisplayName = "曦月平台";
            sys.Save();
        }
        ...
        // 连接字符串
        DAL.ConnStrs.TryAdd("Mdp", "MapTo=Membership");
        ...
        {
            options.DefaultScheme = "Mdp";
            options.DefaultAuthenticateScheme = "Mdp";
            options.DefaultChallengeScheme = "Mdp";
            options.DefaultSignInScheme = "Mdp";
        }).AddCookie("Mdp", options =>
        {
        ...
        //// 注册曦月默认UI
        //services.AddMdpDefaultUI();

        // 配置跨域处理，允许所有来源
        // CORS，全称 Cross-Origin Resource Sharing （跨域资源共享），是一种允许当前域的资源能被其他域访问的机制
        var set = Setting.Current;
        if (set.CorsOrigins == "*")
            services.AddCors(options => options.AddPolicy("Mdp_cors", builder =>
            {
                builder.AllowAnyOrigin().AllowAnyHeader();
            }));
        else if (!set.CorsOrigins.IsNullOrEmpty())
            services.AddCors(options => options.AddPolicy("Mdp_cors", builder => builder.WithOrigins(set.CorsOrigins)));
        ...
        XTrace.WriteLine("{0} End   配置曦月 {0}", new String('=', 32));
    ...
    private static List<Assembly> FindAllArea()
    {
        ...
        // 反射 XML.MDP.*.dll
        foreach (var item in ".".AsDirectory().GetFiles("XML.MDP.*.dll"))
        {
            var asm = Assembly.LoadFile(item.FullName);
            if (!list.Contains(asm) && !list.Any(e => e.FullName == asm.FullName))
            {
        ...
        list.Sort((x, y) =>
        {
            ...
            // 对程序集引用进行排序时，不能使用全名，当曦月更新而APP没有重新编译时，版本的不同将会导致全名不同，无法准确进行排序
    ...
    #region 使用曦月
    /// <summary>使用曦月，放在UseEndpoints之前，自动探测是否UseRouting</summary>
    /// <param name="app"></param>
    /// <param name="env"></param>
    /// <returns></returns>
    public static IApplicationBuilder UseMdp(this IApplicationBuilder app, IWebHostEnvironment env)
    {
        var provider = app.ApplicationServices;
        using var span = DefaultTracer.Instance?.NewSpan(nameof(UseMdp));

        XTrace.WriteLine("{0} Start 初始化曦月 {0}", new String('=', 32));
        ...
        if (!set.CorsOrigins.IsNullOrEmpty()) app.UseCors("Mdp_cors");
        ...
        if (env != null) app.UseMdpDefaultUI(env);

        // 设置默认路由。如果外部已经执行 UseRouting，则直接注册
        app.UseRouter(endpoints =>
        {
            XTrace.WriteLine("注册曦月区域路由");
        ...
        AreaBase.RegisterArea<Mdp.MdpArea>();
        ...
        XTrace.WriteLine("{0} End   初始化曦月 {0}", new String('=', 32));
    ...
    /// <summary>使用曦月首页</summary>
    /// <param name="app"></param>
    /// <returns></returns>
    public static IApplicationBuilder UseMdpHome(this IApplicationBuilder app)
    {
        app.UseRouter(endpoints =>
        {
            endpoints.MapControllerRoute(
                "Default",
                "{controller=MdpHome}/{action=Index}/{id?}"
                );
        });
    ...
    {
        // 消费StarWeb服务地址，如果未接入平台，这里也没必要获取这个地址
