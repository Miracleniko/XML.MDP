...
public static class PathHelper
{
    ...
    static PathHelper()
    {
        ...
        // 环境变量
        if (dir.IsNullOrEmpty()) dir = XML.Core.Runtime.GetEnvironmentVariable("BasePath");
    ...
    /// <returns></returns>
    public static long OpenRead(this FileInfo file, bool compressed, Action<Stream> func)
    {
        if (compressed)
        {
            using var fs = file.OpenRead();
    ...
    /// <param name="func">要对文件流操作的委托</param>
    /// <returns></returns>
    public static long OpenWrite(this FileInfo file, bool compressed, Action<Stream> func)
    {
    ...
    public static void Extract(this FileInfo fi, string destDir, bool overwrite = false)
    {
