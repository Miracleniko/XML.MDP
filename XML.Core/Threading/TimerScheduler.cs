...
public class TimerScheduler
{
    ...
    private TimerEV[] Timers = global::System.Array.Empty<global::XML.Core.Threading.TimerEV>();
    #endregion

    /// <summary>把定时器加入队列</summary>
    /// <param name="timer"></param>
    public void Add(TimerEV timer)
    {
        ...
        lock (this)
        {
            var list = new List<TimerEV>(Timers);
    ...
    public void Remove(TimerEV timer, String reason)
    {
    ...
    /// <returns></returns>
    private Boolean CheckTime(TimerEV timer, Int64 now)
    {
    ...
    {
        if (state is not TimerEV timer) return;

        TimerEV.Current = timer;
    ...
    private void OnExecuted(TimerEV timer, Int32 ms)
    {
        if (timer.Cost == 0)
            timer.Cost = ms;
        else
    ...
    private void OnFinish(TimerEV timer)
    {
