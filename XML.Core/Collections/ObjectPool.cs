...
public class ObjectPool<T> : DisposeBase, IPool<T>
{
    ...
    private void StartTimer()
    {
        ...
        lock (this)
        {
            ...
            _timer = new TimerEV(Work, null, 5000, 5000) { Async = true };
        }
    }
    ...
    private void Work(Object state)
    {
        ...
        if (!_busy.IsEmpty)
        {
            var exp = TimerEV.Now.AddSeconds(-AllIdleTime);
            foreach (var item in _busy)
            {
        ...
        {
            var exp = TimerEV.Now.AddSeconds(-IdleTime);
            // 移除扩展空闲集合里面的超时项
